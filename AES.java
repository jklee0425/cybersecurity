...
public class AES {
	...
	public static void invertByteSub(byte[][] bytes) {
		for(int i = 0; i < bytes.length; i++) {
			for(int j = 0; j < bytes.length; j++) {
				bytes[i][j] = (byte) invertSbox[bytes[i][j]/16][bytes[i][j]%16];
			}
		}
	}
	
	public static void leftShift(byte[] bytes) {
		byte tmp = bytes[0];
	...
	public static void rightShift(byte[] bytes) {
	...
	public static void rightLeft(byte[] bytes) {
		byte tmp;
		for(int i = 0; i < 2; i++) {
			tmp = bytes[i + 2];
			bytes[i + 2] = bytes[i];
	...
	public static void shiftRows(byte[][] subbedBytes) {
	...
	public static void invertShiftRows(byte[][] subbedBytes) {
		for(int i = 0; i < subbedBytes.length; i++) {
			if(i % 4 == 3) {
				rightShift(subbedBytes[i]);
			}
	...
	public static void mixColumns(byte[][] bytes) {
		byte[][] retval = new byte[bytes.length][bytes[0].length];
	...
	public static void invertMixColumns(byte[][] bytes) {
	...
	//add extra bytes
	public static void padding(byte[][] arr, int length) {
		if(length % 4 == 0) {
			return;
		}
	...
	public static void byteToHex(byte[][] arr) {
		for(int i = 0; i < arr.length; i++) {
